...
def step_command():
    ...
    try:
        if parts[0] == "add":  # R-Type add
            ...
            label = parts[1]
            if label in labels:
                registers["$ra"] = pc + 1  # Geri dönüş adresini kaydet (bir sonraki komutun adresi)
                pc = labels[label]        # Hedef etikete atla
                realistic_pc = pc * 4     # Realistic PC'yi güncelle
                update_pc_display()
                update_register_display()
                update_machine_code_display()  # Machine Code alanını güncelle
                ...
                result_label.config(text=f"Etiket bulunamadı: {label}", fg="red")
            return  # "jal" sonrası başka işlem yapılmaz.

        elif parts[0] == "jr":  # J-Type jr
            rs = parts[1]
            if rs in registers:
                pc = registers[rs]  # $ra'daki adrese geri dön
                realistic_pc = pc * 4  # Realistic PC'yi güncelle
                update_pc_display()
                ...
                # Geri dönülen komut çalıştırıldıktan sonra program sonlanmalı
                if pc < len(instruction_memory):
                    next_instruction = fetch_instruction(pc)  # Sıradaki komutu al
                    if next_instruction and next_instruction.strip():  # Eğer geçerli bir komut varsa
                        step_command()  # Bir sonraki komutu çalıştır
                        result_label.config(text="Program sonlandı.", fg="green")  # Programın sonlandığını yazdır
                    else:
                        result_label.config(text="Program sonlandı, geçerli komut yok.", fg="green")
                else:
                    result_label.config(text="Program sonlandı, geçerli komut yok.", fg="green")
            else:
                result_label.config(text=f"Register bulunamadı: {rs}", fg="red")
            return
...
def run_command():
    global pc, realistic_pc
    visited = set()  # Sonsuz döngü kontrolü için ziyaret edilen komutları takip eder
    while pc < len(instruction_memory):  # Program Counter, komut belleğinin sınırları içinde
        ...
        visited.add(pc)  # Mevcut PC'yi ziyaret edilenler listesine ekle
        instruction = fetch_instruction(pc)  # Instruction Memory'den komut al
        if instruction is None or instruction.strip() == "":  # Eğer komut yoksa programı sonlandır
            break
        ...
        try:
            if parts[0] == "add":  # R-Type add
                ...
                if label in labels:
                    update_register_display()
                    pc = labels[label]  # Etikete dallan
                    realistic_pc = pc * 4  # Realistic PC'yi güncelle
                    continue
                ...
                if label in labels:
                    update_register_display()
                    registers["$ra"] = pc + 1  # Geri dönüş adresini kaydet
                    pc = labels[label]  # Etikete dallan
                    realistic_pc = pc * 4  # Realistic PC'yi güncelle
                    continue
                ...
                rs = parts[1]
                if rs in registers:
                    target_pc = registers[rs]
                    if 0 <= target_pc < len(instruction_memory):  # Geçerli bir satırı işaretliyor mu?
                        pc = target_pc  # `$ra` içeriğine dallan
                        realistic_pc = pc * 4  # Realistic PC'yi güncelle
                        continue
                    else:
                        result_label.config(text=f"Geçersiz $ra adresi: {target_pc}", fg="red")
                        break
        ...
        # Tkinter GUI'nin yanıt vermesi için güncelleme
        update_pc_display()
        update_register_display()
        update_memory_display()
        update_machine_code_display()
        root.update()  # Tkinter arayüzünü güncelle
...
def update_pc_display():
    realistic_pc_value_label.config(text=f"{realistic_pc:03}")  # Sadece PC değerini göster
...
root.configure(bg='#87CEEB')  # Açık mavi arka plan
...
input_frame = tk.Frame(main_frame, bg='#FFB6C1', relief="ridge", bd=2)  # Açık pembe
input_frame.grid(row=0, column=0, padx=15, sticky="nsew")

tk.Label(input_frame, text="MIPS Komutları", font=('Arial', 12, 'bold'), bg='#FFB6C1').pack(anchor="w", pady=5)
input_text = tk.Text(input_frame, width=40, height=20, font=('Consolas', 11), bg='white')
input_text.pack(padx=5)

button_frame = tk.Frame(input_frame, bg='#FFB6C1')
...
step_button = tk.Button(button_frame, text="Step", command=step_command,
                       bg='#2196F3', fg='white', font=('Arial', 10, 'bold'),
                       width=12, height=1)
step_button.pack(side=tk.LEFT, padx=5)

load_button = tk.Button(button_frame, text="Load", command=load_all,
                       bg='#FF9800', fg='white', font=('Arial', 10, 'bold'),
                       width=12, height=1)
load_button.pack(side=tk.LEFT, padx=5)

# Register alanı için kaydırılabilir pencere
register_frame = tk.Frame(main_frame, bg='#98FB98', relief="ridge", bd=2)  # Açık yeşil
register_frame.grid(row=0, column=1, padx=15, sticky="nsew")

tk.Label(register_frame, text="Registers", font=('Arial', 12, 'bold'), bg='#98FB98').pack(anchor="w", pady=5, padx=5)

# Canvas ve Scrollbar ekleme
canvas = tk.Canvas(register_frame, bg='#98FB98')
...
register_labels = []
for i, name in enumerate(register_names):
    frame = tk.Frame(scrollable_frame, bg='#98FB98')
    frame.pack(fill='x', padx=5, pady=2)
    
    reg_label = tk.Label(frame, text=name, width=20, anchor="w", 
                        font=('Consolas', 10), bg='#98FB98')
    reg_label.pack(side=tk.LEFT)
    
    value_label = tk.Label(frame, text="0", width=10, anchor="w",
                          font=('Consolas', 10), bg='#98FB98')
...
# Memory alanları
memory_frame = tk.Frame(main_frame, bg='#DDA0DD', relief="ridge", bd=2)  # Açık mor
memory_frame.grid(row=0, column=2, padx=15, sticky="nsew")

tk.Label(memory_frame, text="Data Memory", font=('Arial', 12, 'bold'), bg='#DDA0DD').pack(anchor="w", pady=5, padx=5)
memory_text = tk.Text(memory_frame, width=30, height=20, font=('Consolas', 11), bg='#FAFAFA')
memory_text.pack(padx=5, pady=5)

instruction_memory_frame = tk.Frame(main_frame, bg='#FFA07A', relief="ridge", bd=2)  # Açık turuncu
instruction_memory_frame.grid(row=0, column=3, padx=15, sticky="nsew")

tk.Label(instruction_memory_frame, text="Instruction Memory", font=('Arial', 12, 'bold'), bg='#FFA07A').pack(anchor="w", pady=5, padx=5)
instruction_memory_text = tk.Text(instruction_memory_frame, width=30, height=20, font=('Consolas', 11), bg='#FAFAFA')
instruction_memory_text.pack(padx=5, pady=5)

# Program Counter
pc_frame = tk.Frame(main_frame, bg='#F0E68C', relief="ridge", bd=2)  # Açık sarı
pc_frame.grid(row=1, column=2, columnspan=2, pady=15, sticky="ew")

tk.Label(pc_frame, text="Program Counter:", font=('Arial', 11, 'bold'), bg='#F0E68C').pack(side=tk.LEFT, padx=10)
realistic_pc_value_label = tk.Label(pc_frame, text="000", font=('Consolas', 11), bg='#F0E68C')
realistic_pc_value_label.pack(side=tk.LEFT)

# Machine Code ekranı
machine_code_frame = tk.Frame(main_frame, bg='#B0E0E6', relief="ridge", bd=2)  # Açık mavi
machine_code_frame.grid(row=1, column=0, columnspan=2, pady=15, sticky="ew")

tk.Label(machine_code_frame, text="Machine Code", font=('Arial', 12, 'bold'), bg='#B0E0E6').pack(anchor="w", pady=5, padx=5)
machine_code_text = tk.Text(machine_code_frame, width=60, height=10, font=('Consolas', 11), bg='#FAFAFA')
machine_code_text.pack(padx=5, pady=5)
...
# Sonuç mesajı
result_label = tk.Label(root, text="", font=('Arial', 11), bg='#87CEEB')
